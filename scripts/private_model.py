...
def _person_internal_key(row: pd.Series) -> str:
    ...
    if rut and rut != "0":
        return f"RUT:{rut}"
    if clean(row.get("rut_persona", "")) in {"0-E", "0-E (Extranjero)"}:
        # No cross-company identity claim without documentary evidence. Exact source
        # observations remain stable, but homonyms are never merged by name alone.
        fields = ("rut_empresa", "nombre_original", "cargo", "fecha_nombramiento", "source_url")
        return "UNRESOLVED:" + json.dumps([clean(row.get(k, "")) for k in fields], ensure_ascii=False)
    raise ValueError("Identidad sin RUT: requiere resolución privada explícita antes de publicar")


def _public_person_ids(directors: pd.DataFrame, registry: Path) -> dict[str, str]:
    # Random IDs have no mathematical relationship to the private identifier.
    registry.parent.mkdir(parents=True, exist_ok=True)
    mapping = json.loads(registry.read_text(encoding="utf-8")) if registry.exists() else {}
    if len(set(mapping.values())) != len(mapping) or any(
        not re.fullmatch(r"p_[0-9a-f]{32}", value) for value in mapping.values()
    ):
        raise ValueError("Correspondencia privada inválida")
    for key in sorted(directors["_person_key"].unique()):
        mapping.setdefault(key, "p_" + uuid.uuid4().hex)
...
def load_model(run_dir: str | Path, registry: Path | None = None) -> dict[str, pd.DataFrame | str | Path]:
    ...
    # Optional curated mapping, kept beside the private registry. Keys are observation
    # identities; values are verified RUT keys or opaque LOCAL:<uuid> identities.
    private_root = root.parent if root.parent.name == "private_data" else root.parent / "private_data"
    registry = registry or private_root / "person_ids.json"
    resolutions_path = registry.parent / "identity_resolutions.json"
    if resolutions_path.exists():
        resolutions = json.loads(resolutions_path.read_text(encoding="utf-8"))
        for key, entry in resolutions.items():
            if not key.startswith("UNRESOLVED:") or not entry.get("evidence") or not re.fullmatch(r"RUT:[0-9K]+|LOCAL:[0-9a-f]{32}", entry.get("identity", "")):
                raise ValueError("Resolución privada sin evidencia o identidad válida")
        raw["_person_key"] = raw["_person_key"].map(lambda key: resolutions.get(key, {}).get("identity", key))
    ...
    person_map = _public_person_ids(raw, registry)
